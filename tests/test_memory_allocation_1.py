...
class MemoryAllocation1TestCase(unittest.TestCase):
    def test_default_full_classification(self):
        ...
        tracemalloc.stop()
        self.assertFalse(
            tracemalloc.is_tracing(), msg="tracemalloc should not have been started yet"
        )
        tracemalloc.start()
        ...
        self.assertLess(
            b_to_mib(sum(s.size_diff for s in s2.compare_to(s1, key_type="lineno"))),
            1.0,
        )
        ...
        self.assertLess(
            b_to_mib(sum(s.size_diff for s in s4.compare_to(s3, key_type="lineno"))),
            0.5,
        )
